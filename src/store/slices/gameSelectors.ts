...
export const selectIsCellPlayable = (x: number, y: number) =>
	createSelector([selectPlayablePositions], (playablePositions) =>
		playablePositions.some((pos) => pos.x === x && pos.y === y),
	);

// Sélecteur pour récupérer toutes les cellules d'un joueur
export const selectPlayerCells = (playerId: number) =>
...
export const selectCurrentSequence = createSelector(
	...
	(state) => {
		if (
			...
			state.currentSequenceIndex >= state.sequencesToPlace.length
		) {
			return null;
		}
...
export const selectPlayerBaseHp = (playerId: number) =>
	createSelector(
		[selectBaseHpCache],
