 ...
 */
export const markSequenceAsCompleted = (
	state: GameState,
...
export const calculatePlayablePositions = (state: GameState) => {
	...
	if (
		...
		const currentPos = {
			x:
				state.sequenceStartPosition.x +
				state.sequenceDirection.dx * lastBlockIndex,
			y:
				state.sequenceStartPosition.y +
				state.sequenceDirection.dy * lastBlockIndex,
		};
