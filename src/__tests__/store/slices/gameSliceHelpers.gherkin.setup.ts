...
export const createGameSliceHelpersRunner = () => {
	return createGherkinRunner({
		given: {
			...
			"a sequence of type {string} with {number} blocks": (
				...
				state.sequencesToPlace = [
					{ type: type as "attack" | "defense" | "destroy", nbBlocks },
				];
				state.currentSequenceIndex = 0;
				state.currentBlockType = type as
					| "attack"
					| "defense"
					| "destroy"
					| null;
			...
			"blocks placed in sequence: {number}": (
				count: number,
				state: GameState,
			): GameState => {
			...
			"sequence start position ({number}, {number})": (
				x: number,
				y: number,
				state: GameState,
			): GameState => {
			...
			"sequence direction ({number}, {number})": (
				dx: number,
				dy: number,
				state: GameState,
			): GameState => {
			...
			"base HP cache for player {number}: {number}": (
				player: number,
				hp: number,
				state: GameState,
			): GameState => {
		...
		when: {
			"I capture territories for player {number}": (
				player: number,
				board: Cell[][],
			): void => {
				captureTerritories(board, player);
			},
			"I calculate playable positions": (
				state: GameState,
			...
			"I calculate base HP for player {number}": (
				player: number,
				board: Cell[][],
			): number => {
			...
			"I update base HP cache at position ({number}, {number})": (
				x: number,
				y: number,
				state: GameState,
			): void => {
		...
		then: {
			...
			"remaining blocks should be {number}": (
				expected: number,
				actual: number,
			): void => {
				expect(actual).toBe(expected);
			},
			"base HP for player {number} should be {number}": (
				player: number,
				expected: number,
				actual: number,
			): void => {
			...
			"alive players should be": (
				expected: number[],
				actual: number[],
			): void => {
				expect(actual).toEqual(expected);
			},
			"base HP cache for player {number} should be {number}": (
				player: number,
				expected: number,
				state: GameState,
			): void => {
			...
			"eliminated players should be": (
				expected: number[],
				state: GameState,
			): void => {
				expect(state.eliminatedPlayers).toEqual(expected);
			},
			"winner should be {number}": (
			...
			"base HP cache should be": (
				expected: Record<number, number>,
				state: GameState,
			): void => {
				expect(state.baseHpCache).toEqual(expected);
			},
			"playable positions should be": (
