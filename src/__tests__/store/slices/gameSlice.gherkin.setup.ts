...
export const createGameSliceRunner = () => {
	return createGherkinRunner({
		given: {
			...
			"dice result is {number} and {number}": (
				die1: number,
				die2: number,
				state: GameState,
			): GameState => {
			...
			"sequence direction is ({number}, {number})": (
				dx: number,
				dy: number,
				state: GameState,
			): GameState => {
			...
			"a cell at position ({number}, {number}) owned by player {number} with territory {number} and zone {number}":
				(
					...
					const newBoard = state.board.map((row, i) =>
						i === x
							? row.map((cell, j) =>
									j === y ? createCell({ owner, territory, zone }) : cell,
								)
							: row,
					);
				...
				(
					...
					const newBoard = state.board.map((row, i) =>
						i === x
							? row.map((cell, j) =>
									j === y
										? createCell({ type: "base", owner: player, hp })
										: cell,
								)
							: row,
					);
			...
			"current player is {number}": (
				player: number,
				state: GameState,
			): GameState => {
				return { ...state, currentPlayer: player };
			},
			"number of players is {number}": (
				nbPlayers: number,
				state: GameState,
			): GameState => {
		...
		when: {
			"I dispatch setDiceResult with {number} and {number}": (
				die1: number,
				die2: number,
				state: GameState,
			): GameState => {
			...
			"I dispatch selectSequence {number}": (
				index: number,
				state: GameState,
			): GameState => {
				return gameReducer(state, selectSequence(index));
			},
			"I dispatch setSequenceStartPosition ({number}, {number})": (
				x: number,
				y: number,
				state: GameState,
			): GameState => {
			...
			"I dispatch setSequenceDirection ({number}, {number})": (
				dx: number,
				dy: number,
				state: GameState,
			): GameState => {
		...
		then: {
			"dice result should be {number} and {number}": (
				die1: number,
				die2: number,
				state: GameState,
			): void => {
			...
			"dice used should be {array}": (
				expected: number[],
				state: GameState,
			): void => {
				expect(state.diceUsed).toEqual(expected);
			},
			"sequences to place should be": (
				expected: BlockToPlace[],
				state: GameState,
			): void => {
				expect(state.sequencesToPlace).toEqual(expected);
			},
			"current sequence index should be {number}": (
				expected: number,
				state: GameState,
			): void => {
				expect(state.currentSequenceIndex).toBe(expected);
			},
			"current block type should be {string}": (
				expected: string | null,
				state: GameState,
			): void => {
				expect(state.currentBlockType).toBe(expected);
			},
			"blocks placed in sequence should be {number}": (
				expected: number,
				state: GameState,
			): void => {
				expect(state.blocksPlacedInSequence).toBe(expected);
			},
			"sequence start position should be ({number}, {number})": (
				x: number,
				y: number,
				state: GameState,
			): void => {
			...
			"sequence direction should be ({number}, {number})": (
				dx: number,
				dy: number,
				state: GameState,
			): void => {
			...
			"completed sequences should be {array}": (
				expected: number[],
				state: GameState,
			): void => {
				expect(state.completedSequences).toEqual(expected);
			},
			"playable positions should have at least {number} position(s)": (
				minCount: number,
				state: GameState,
			): void => {
			...
			"current player should be {number}": (
				expected: number,
				state: GameState,
			): void => {
			...
			"cell at ({number}, {number}) should have owner {number}": (
				x: number,
				y: number,
				owner: number,
				state: GameState,
			...
			"cell at ({number}, {number}) should have HP {number}": (
				x: number,
				y: number,
				hp: number,
				state: GameState,
			...
			"cell at ({number}, {number}) should have territory {number}": (
				x: number,
				y: number,
				territory: number,
				state: GameState,
