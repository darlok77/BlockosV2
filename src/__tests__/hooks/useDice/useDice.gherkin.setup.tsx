...
export const createUseDiceRunner = () => {
	return createGherkinRunner({
		...
		then: {
			"dice result should be dispatched with values between 1 and 6": (
				context: HookTestContext,
			): void => {
				...
				const lastCall =
					context.dispatchSpy.mock.calls[
						context.dispatchSpy.mock.calls.length - 1
					][0];
			...
			"setDiceResult should be dispatched": (
				context: HookTestContext,
			): void => {
				expect(context.dispatchSpy).toHaveBeenCalledWith(
					expect.objectContaining({
			...
			"setDiceUsedValue should not be dispatched": (
				context: HookTestContext,
			): void => {
