...
export const createUseCellActionsRunner = () => {
	return createGherkinRunner({
		given: {
			"a game state with dice used {array}": (
				diceUsed: number[],
			): HookTestContext => {
			...
			"current sequence is null": (
				context: HookTestContext,
			): HookTestContext => {
				return { ...context, currentSequence: null };
			},
			"blocks placed in sequence is {number}": (
				count: number,
				context: HookTestContext,
			): HookTestContext => {
			...
			"can place block is {boolean}": (
				canPlace: boolean,
				context: HookTestContext,
			): HookTestContext => {
				return { ...context, canPlaceBlock: canPlace };
			},
			"is playable is {boolean}": (
				playable: boolean,
				context: HookTestContext,
			): HookTestContext => {
		...
		then: {
			...
			"incrementBlocksPlaced should be dispatched": (
				context: HookTestContext,
			): void => {
				expect(context.dispatchSpy).toHaveBeenCalledWith(
					expect.objectContaining({
			...
			"setSequenceStartPosition should not be dispatched": (
				context: HookTestContext,
			): void => {
			...
			"completeSequence should be dispatched": (
				context: HookTestContext,
			): void => {
				expect(context.dispatchSpy).toHaveBeenCalledWith(
					expect.objectContaining({
			...
			"playablePositions should be dispatched": (
				context: HookTestContext,
			): void => {
				expect(context.dispatchSpy).toHaveBeenCalledWith(
					expect.objectContaining({
			...
			"playablePositions should not be dispatched": (
				context: HookTestContext,
			): void => {
				expect(context.dispatchSpy).not.toHaveBeenCalledWith(
					expect.objectContaining({
