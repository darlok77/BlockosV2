...
export const useGameLogic = () => {
	...
	useEffect(() => {
		...
		const allSequencesCompleted =
			diceUsed.length > 0 &&
			areAllSequencesCompleted(completedSequences, sequencesToPlace.length);

		if (allSequencesCompleted) {
			console.log(
				"Toutes les séquences ont été placées, passage au joueur suivant...",
			);
			dispatch(passTurn());
		}
	}, [
		completedSequences.length,
		sequencesToPlace.length,
		diceUsed.length,
		dispatch,
		sequencesToPlace,
		completedSequences,
